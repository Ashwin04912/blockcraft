...
<section class="w-100">
    ...
    <div class="row row-cols-2 row-cols-lg-4 g-3">
        @forelse($stats as $stat)
            <div class="col">
                <div class="card h-100 border-0 shadow-sm rounded-4 text-center transition group-hover-translate-y {{ $padding }}">
                    <div class="card-body p-0">
                        @if(!empty($stat['icon']))
                            <div class="d-flex justify-content-center mb-2 text-success">
                                @if($stat['icon'] === 'users')
                        ...
                        <div class="{{ $valSize }} fw-bold text-success tracking-tight">
                            {{ $stat['value'] ?? '—' }}
                        </div>
                        <div class="small text-secondary fw-semibold mt-2 text-uppercase" style="letter-spacing: 0.05em;">
                            {{ $stat['label'] ?? '' }}
                        </div>
                    </div>
                </div>
