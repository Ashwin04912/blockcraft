    ...
    <div class="row g-4">
        @forelse($cards as $card)
            <div class="col-12 col-sm-6 col-lg-4">
                <div class="card h-100 border-0 shadow-sm rounded-4 overflow-hidden transition group">
                    ...
                    @else
                        <div class="w-100 {{ $imgH }} position-relative overflow-hidden bg-primary bg-opacity-10 d-flex align-items-center justify-content-center">
                            <svg xmlns="http://www.w3.org/2000/svg" width="64" height="64" class="text-primary opacity-50" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    ...
                    <div class="card-body d-flex flex-column {{ $padding }}">
                        @if(!empty($card['title']))
                            <h3 class="fw-bold text-dark {{ $titleSize }} mb-2 tracking-tight">{{ $card['title'] }}</h3>
                        @endif
