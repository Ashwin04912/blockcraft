// ════════════════════ VISUAL EDITOR ════════════════════
// Reads server-rendered config from window.EDITOR_CONFIG (set by an inline
// script in admin/preview.blade.php) instead of Blade-interpolating values
// directly into this file, so this stays a plain static JS asset.

const CSRF      = document.querySelector('meta[name="csrf-token"]').content;
const BASE_URL  = window.EDITOR_CONFIG.baseUrl;
const SITE_BASE = window.EDITOR_CONFIG.siteBase;
...
async function setBackground(value, btnEl) {
    try {
        const res = await api('PATCH', `${SITE_BASE}/background`, { background_color: value });
        if (!res.ok) {
            toast('Failed to update background', 'error');
            return;
        }
        document.querySelectorAll('.bg-swatch-btn').forEach(b => b.style.borderColor = '#dee2e6');
        btnEl.style.borderColor = '#6366f1';
        document.getElementById('bg-picker-swatch').style.background = value;
        document.getElementById('bg-picker').style.display = 'none';
        toast('Background updated — view on Live Site', 'success');
    } catch (e) {
...
function openEditor(id) {
    ...
    // Background style — every block type renders this now. footer's
    // unconfigured default is 'dark' (matches its hardcoded historical
    // look); everything else defaults to 'light'.
    const bgStyle = block.config?.bg_style ?? (block.type === 'footer' ? 'dark' : 'light');
...
// ─── Add Block modal: dynamic row helpers ──────────────────────
...
function switchAddModalType(type) {
    ...
    // Every block type renders bg_style now. footer's natural default is
    // 'dark' (its hardcoded historical look); everything else is 'light'.
    const bgStyleSelect = document.getElementById('add-bg-style');
    if (bgStyleSelect) {
        bgStyleSelect.value = (type === 'footer') ? 'dark' : 'light';
    }
}
...
// Expose handlers referenced via inline onclick="" in Blade-rendered HTML —
// they need to live on window since this file is loaded as an ES module
// (module scope isn't global scope).
Object.assign(window, {
    toggleBgPicker, setBackground, openEditor, closeSidebar,
    selectSize, selectAddSize, addSbCardItem, addSbListItem, addSbStatItem,
    addSbNavItem, addSbFooterLink, addSbSocialLink, removeSbRow,
    saveSidebar, quickToggle, deleteBlock, addCardItemModal,
    addHeaderNavItemModal, addFooterLinkModal, addFooterSocialModal,
    openAddModal, closeAddModal, switchAddModalType, addListItemModal,
    addStatItemModal, removeRow, submitAddBlock,
});
